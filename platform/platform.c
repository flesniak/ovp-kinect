...
void usage( char* argv0 ) {
  ...
  fprintf( stderr, "  -k                   | enable kinect\n" );
...
void parseOptions( int argc, char** argv ) {
  ...
  while( ( c = getopt( argc, argv, "vt:m:i:w:d:k" ) ) != -1 )
    switch( c ) {
      case 'v':
        options.verbosity++;
        break;
      ...
      case 'k':
        if( options.memorySize != UINT_MAX ) {
          fprintf( stderr, "Options -k and -m are not compatible\n" );
          usage( argv[0] );
        }
        options.kinect = 1;
        break;
...
int main( int argc, char** argv ) {
  ...
  if( options.verbosity > 1 ) {
    icmInitPlatform( ICM_VERSION, ICM_VERBOSE | ICM_STOP_ON_CTRLC, 0, 0, "kinectdemo" );
  } else
    icmInitPlatform( ICM_VERSION, ICM_INIT_DEFAULT | ICM_STOP_ON_CTRLC, 0, 0, "kinectdemo" );
  ...
  if( options.display || options.kinect ) {
    //bus
    ...
    //only works if DVI_BASE_ADDRESS < KINECT_REGS_DEFAULT_ADDRESS
    //CASE 1: capture only
    ...
    if( options.display || options.kinect ) {
      ...
      if( options.kinect ) {
        mem[1] = icmNewMemory("mem2", ICM_PRIV_RWX, KINECT_REGS_DEFAULT_ADDRESS-(DVI_BASE_ADDRESS+DVI_CONTROL_REGS_SIZE)-1);
        mem[2] = icmNewMemory("mem3", ICM_PRIV_RWX, UINT_MAX-(KINECT_REGS_DEFAULT_ADDRESS+KINECT_REGS_SIZE)-1);
        icmConnectMemoryToBus(bus1, "port3", mem[2], KINECT_REGS_DEFAULT_ADDRESS+KINECT_REGS_SIZE);
      } else
      ...
      mem[0] = icmNewMemory("mem1", ICM_PRIV_RWX, KINECT_REGS_DEFAULT_ADDRESS-1);
      mem[1] = icmNewMemory("mem2", ICM_PRIV_RWX, UINT_MAX-(KINECT_REGS_DEFAULT_ADDRESS+KINECT_REGS_SIZE)-1);
      icmConnectMemoryToBus(bus1, "port2", mem[1], KINECT_REGS_DEFAULT_ADDRESS+KINECT_REGS_SIZE);
    }
